...
def _is_collectible_vehicle(title_text: str | None, year: int | None) -> bool:
    ...
    muscle_cars = ["mustang", "camaro", "corvette", "charger", "challenger", "chevelle", 
                   "firebird", "gto", "barracuda", "nova ss", "impala", "barracuda"]
    ...
    jdm_legends = ["supra", "skyline", "silvia", "rx-7", "nsx", "300zx", "celica", "mr2", "rx7", "s13", "s14"]
    if any(m in text for m in jdm_legends):
        if year and 1985 <= year <= 2005:
            return True
    
    # 3. European Exotics/Sports
    exotic_brands = ["porsche", "ferrari", "lamborghini", "mclaren", "aston martin", "maserati", "lotus"]
